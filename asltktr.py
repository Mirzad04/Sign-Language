...
class ASLRecognizerApp:
    ...
    def setup_ui(self):
        self.app = tk.Tk()
        self.app.title("ASL Recognition")
        self.app.geometry("800x600")

        self.video_label = tk.Label(self.app)
        self.video_label.pack(pady=10)

        self.word_label = tk.Label(self.app, text="Current Word: ", font=("Arial", 18))
        self.word_label.pack(pady=10)

        self.predicted_label_label = tk.Label(self.app, text="Prediction: ", font=("Arial", 18))
        self.predicted_label_label.pack(pady=10)

        self.start_button = tk.Button(self.app, text="Start Prediction", command=self.start_prediction)
        self.start_button.pack(pady=10)

        self.delete_button = tk.Button(self.app, text="Delete Last Letter", command=self.delete_letter)
        ...
        while self.is_predicting:
            ...
            if self.predicted_count >= 15:
                if self.predicted_label == 'Space':
                    ...
                    self.word += " "  # Append whitespace for 'Nothing'
                else:
    ...
    def update_display(self, frame):
        ...
        if self.predicted_label not in ['Space', 'Delete']:
            ...
            self.predicted_label_label.configure(text="Prediction: ")  # Clear prediction label for 'Space' and 'Delete'
